...
bool Yolov8_Det_DFL::ReadModel(Net& net, string& netPath, bool isCuda = false) {
	try {
		net = readNet(netPath);
...
static void sigmoid(const cv::Mat& input, cv::Mat& dest){
     cv::exp(input, dest);
     dest = 1.0 / (1.0 + dest);
}
...
)
{
    ...
    std::cout << " output dimes:" << feat_blob[0].dims<< std::endl;
	...
	//const int num_class = num_feat_w - 4 * reg_max;
	...
	for (int i = 0; i < num_grid_x; i++)
	{
		for (int j = 0; j < num_grid_y; j++)
		{
            ...
            // cls-dim:[1, resolution_w, resolution_h, num_class]
            int id = feat_blob[1].step[0] * b + feat_blob[1].step[1] * i + feat_blob[1].step[2] * j; // + w * a.step[3];
            const float* matat_score = (float*)(feat_blob[1].data + id);
            ...
            // box-dim:[1, resolution_w, resolution_h, 4*reg_max]
            int id_box = feat_blob[0].step[0] * b + feat_blob[0].step[1] * i + feat_blob[0].step[2] * j; // + w * a.step[3];
            const float* matat_dbox = (float*)(feat_blob[0].data + id_box);
...
bool Yolov8_Det_DFL::Detect(Mat& srcImg, Net& net, vector<OutputSeg>& output) {
	Mat blob;
	output.clear();
	int col = srcImg.cols;
	int row = srcImg.rows;
    ...
    //vector<string> output_layer_names{"lbox", "lscore", "mbox", "mscore"};
    vector<string> output_layer_names;
    output_layer_names = net.getUnconnectedOutLayersNames();
    std::cout << "model_output_name:" << std::endl;
    for(int i=0; i<output_layer_names.size(); i+=2){
        std::cout << output_layer_names[i] <<" " << output_layer_names[i+1] << endl;
    }
	...
	std::vector<OutputSeg> proposals;
    //const int target_size = 640;
    // stride 8
    {
        std::vector<OutputSeg> objects8;
	    const std::vector<cv::Mat>& feat_blob = {net_output_img[0], net_output_img[1]};
        generate_proposals(8, feat_blob, _classThreshold, objects8);
		proposals.insert(proposals.end(), objects8.begin(), objects8.end());
    }
    // stride 16
    {
        std::vector<OutputSeg> objects16;
	    const std::vector<cv::Mat>& feat_blob = {net_output_img[2], net_output_img[3]};
        generate_proposals(16, feat_blob, _classThreshold, objects16);
		proposals.insert(proposals.end(), objects16.begin(), objects16.end());
    }
    // stride 32
    {
        std::vector<OutputSeg> objects32;
	    const std::vector<cv::Mat>& feat_blob = {net_output_img[4], net_output_img[5]};
        generate_proposals(32, feat_blob, _classThreshold, objects32);
