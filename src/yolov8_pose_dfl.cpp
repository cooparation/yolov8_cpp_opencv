...
bool Yolov8_Pose_DFL::ReadModel(Net& net, string& netPath, bool isCuda = false) {
	try {
		net = readNet(netPath);
...
)
{
   ...
   // }
    ...
    const int kps_size = feat_blob[2].size[3]; // 51=17 X 3
    cout << "kps_size:" << kps_size <<endl;
	...
	for (int i = 0; i < num_grid_x; i++)
	{
		for (int j = 0; j < num_grid_y; j++)
		{
            ...
            // kps: [1, resolution, resolution, 17*3]
            int id_kps = feat_blob[2].step[0] * b + feat_blob[2].step[1] * i + feat_blob[2].step[2] * j; // + w * a.step[3];
            const float* matat_kps = (float*)(feat_blob[2].data + id_kps);
			if (class_score >= prob_threshold){
                ...
                std::vector<float> temp_kps; // kps_size = 17X3 = 51
                for(int k=0; k<kps_size; k+=3){

                    float px = (matat_kps[k]*2. + j)*stride; //x
                    float py = (matat_kps[k+1]*2. + i)*stride; //y
                    float pscore = matat_kps[k+2]; //score
                    pscore = 1 / (1 + cv::exp(-pscore)); // sigmoid
                    //cout << " " << pscore;
                    temp_kps.push_back(px);
                    temp_kps.push_back(py);
                    temp_kps.push_back(pscore);
                }//cout << endl;
                kps.push_back(temp_kps);
...
bool Yolov8_Pose_DFL::Detect(Mat& srcImg, Net& net, vector<OutputSeg>& output) {
	Mat blob;
	output.clear();
	int col = srcImg.cols;
	int row = srcImg.rows;
    ...
    //vector<string> output_layer_names{"lbox", "lkps", "lscore", "mbox", "mkps", "mscore", "sbox", "skps", "sscore"};
    ...
    const std::vector<cv::Mat>& feat_blob = {
        ...
        // middle: box, kps, score
        net_output_img[3], net_output_img[4], net_output_img[5],
        // small: box, kps, score
        net_output_img[6], net_output_img[7], net_output_img[8]
    };
    print_output_dims(feat_blob);

	std::vector<OutputSeg> proposals; // cls, box
    std::vector<vector<float>> kps_proposals; // mask
    //const int target_size = 640;
    // stride 8
    {
        std::vector<OutputSeg> objects8;
	    const std::vector<cv::Mat>& feat_blob = {net_output_img[2], net_output_img[0], net_output_img[1]}; 
        generate_proposals(8, feat_blob, _classThreshold, objects8, kps_proposals);
		proposals.insert(proposals.end(), objects8.begin(), objects8.end());
    }
    // stride 16
    {
        std::vector<OutputSeg> objects16;
	    const std::vector<cv::Mat>& feat_blob = {net_output_img[5], net_output_img[3], net_output_img[4]};
        generate_proposals(16, feat_blob, _classThreshold, objects16, kps_proposals);
		proposals.insert(proposals.end(), objects16.begin(), objects16.end());
    }
    // stride 32
    {
        std::vector<OutputSeg> objects32;
	    const std::vector<cv::Mat>& feat_blob = {net_output_img[8], net_output_img[6], net_output_img[7]};
        generate_proposals(32, feat_blob, _classThreshold, objects32, kps_proposals);
    ...
    std::vector<vector<float>> picked_kps_proposals;
	for (auto i : nms_result_index){
        ...
        picked_kps_proposals.push_back(kps_proposals[i]);
	}
    PoseParams pose_params;
    pose_params.params = params;
    pose_params.srcImgShape = srcImg.size();
    for (int i = 0; i < picked_kps_proposals.size(); ++i) {
        //GetMask2(cv::Mat(picked_kps_proposals[i]).t(), net_output_img[6], output[i], mask_params);
        GetKPS2(picked_kps_proposals[i], output[i], pose_params);
    }
